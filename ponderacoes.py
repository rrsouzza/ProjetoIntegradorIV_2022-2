        ...
        for line in txt_file.readlines():
            ...
            if not keepLooking:
                if line.startswith(f'{index}-'):
                    print('Entrou no startsWith')
                    if line.endswith(';'):
                        print('Entrou no endsWith de cima')
                        keepLooking = False
                        currentTweet = line
                    else:
                        keepLooking = True
                ...
                if line.endswith(';'):
                    print('Entrou no endsWith de baixo')
                    keepLooking = False
            ...
            if keepLooking:
                currentTweet = f'{currentTweet}{line}'
                print(f'---keepLooking --- currentTweet: {currentTweet}')
            else:
                csv_file.write(currentTweet)
                print(f'---notKeepLooking --- currentTweet: {currentTweet}')
                currentTweet = ''
            index += 1
